    ...
    if generate_btn or st.session_state.get("generated"):
        ...
        with st.spinner("正在生成地图..."):
            try:
                ...
                # Use data: URI iframe to avoid Streamlit sandbox restrictions
                # This ensures echarts CDN scripts can load and execute properly
                ...
                st.caption("💡 提示：如地图颜色未加载，请稍等片刻（CDN资源加载中），或直接下载HTML文件在浏览器中打开。")
