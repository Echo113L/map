...
Smart visualMap: auto-detects tiny-value data and switches to
piecewise mode so every province shows a distinct color.
"""
...
def _render_chart_to_html(chart) -> str:
    """Render any pyecharts chart to HTML via temp file."""
...
def _colour_tooltip_style(colors: List[str], unit: str = "") -> str:
    """Return a JS formatter string that shows value + a small colour swatch."""
    unit_suffix = f" {unit}" if unit else ""
    # ECharts formatter function — uses b (name) and c (value) placeholders
    # We use the built-in {b} {c} syntax which pyecharts supports natively
    return "{b}<br/>数值：{c}" + unit_suffix


# ── Smart visualMap factory ─────────────────────────────────────────────


def _ratio_span(vals: List[float]) -> float:
    """max/min ratio; ∞ if min is 0 or negative."""
...
def _span_is_tiny(vals: List[float]) -> bool:
    """
    True when the data range is so narrow that a continuous colour ramp
    makes every province look nearly identical.
    Thresholds:
      - max ≤ 1     AND ratio < 5   → tiny
      - max ≤ 50    AND ratio < 2   → tiny
      - 50 < max    AND ratio < 1.3 → tiny
    """
    if len(vals) < 2:
        return False
...
def _build_visualmap(
    ...
    """
    Build a VisualMapOpts instance that handles both normal
    and micro-value data well.
    """
    if not vals:
    ...
    # ── Narrow-span path: use piecewise buckets ──
    if _span_is_tiny(vals):
        # Build 5–6 evenly-spaced buckets so every colour is used
        n_buckets = min(len(sorted_colors), 6)
        span = mx - mn
        if span == 0:
            span = mx * 0.1 if mx != 0 else 1
        ...
        for i in range(n_buckets - 1):
            ...
            pieces.append(
                {"min": lo, "max": hi, "color": sorted_colors[i] if i < len(sorted_colors) else sorted_colors[-1]}
            )
        # Last piece: up to max
        pieces.append({
    ...
    # ── Normal path: continuous gradient ──
    return opts.VisualMapOpts(
        min_=mn,
        max_=mx,
        range_color=sorted_colors,
...
def _make_map(
    ...
    visualmap = _build_visualmap(vals, range_colors, is_multi=is_multi)

    # Tooltip
    unit_str = f" {unit}" if unit else ""
    tooltip_fmt = "{b}<br/>数值：{c}" + unit_str
